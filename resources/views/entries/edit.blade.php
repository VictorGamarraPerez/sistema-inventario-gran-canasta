...
@section('page-title', 'Editar Entrada de Producto')
    ...
    <div class="col-lg-8 mx-auto">
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body p-4">
                <form action="{{ route('entries.update', $entry) }}" method="POST" id="entryForm">
                    @csrf
                    @method('PUT')
                            ...
                            <select class="form-select @error('supplier_id') is-invalid @enderror" 
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" 
                                            {{ (old('supplier_id', $entry->supplier_id) == $supplier->id) ? 'selected' : '' }}>
                                        {{ $supplier->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="productInfo" class="mt-2" style="display: {{ old('product_id', $entry->product_id) ? 'block' : 'none' }};">
                            <div class="alert alert-info mb-0">
                                <strong>Stock actual:</strong> <span id="currentStock">{{ $entry->product->stock }}</span> unidades
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('document_type') is-invalid @enderror" 
                            ...
                            <option value="Boleta De Compra" {{ old('document_type', $entry->document_type) == 'Boleta De Compra' ? 'selected' : '' }}>Boleta De Compra</option>
                            <option value="Factura" {{ old('document_type', $entry->document_type) == 'Factura' ? 'selected' : '' }}>Factura</option>
                        </select>
                        @error('document_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control @error('observations') is-invalid @enderror" 
                                  ...
                                  name="observations" 
                                  rows="3"
                                  placeholder="Ingrese cualquier observación o nota sobre esta entrada...">{{ old('observations', $entry->observations) }}</textarea>
                        @error('observations')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="alert alert-warning border-0">
                        ...
                        <strong>Nota:</strong> Al modificar esta entrada, el stock del producto se ajustará automáticamente.
                    </div>
...
<script>
    ...
    // Calcular stock inicial si hay producto seleccionado
    ...
    document.getElementById('entryForm').addEventListener('submit', function(e) {
        if (!confirm('¿Está seguro de actualizar esta entrada? El stock del producto se ajustará automáticamente.')) {
            e.preventDefault();
        }
    });
</script>
