        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h4 class="mb-0"><i class="fas fa-sign-out-alt me-2"></i>Registrar Salida de Producto</h4>
                </div>
                <div class="card-body">
                    @if(session('error'))
                    ...
                    <form action="{{ route('exits.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('reason') is-invalid @enderror" 
                                ...
                                <option value="venta" {{ old('reason') == 'venta' ? 'selected' : '' }}>Venta</option>
                                <option value="devolución" {{ old('reason') == 'devolución' ? 'selected' : '' }}>Devolución</option>
                                <option value="pérdida" {{ old('reason') == 'pérdida' ? 'selected' : '' }}>Pérdida</option>
                                <option value="merma" {{ old('reason') == 'merma' ? 'selected' : '' }}>Merma</option>
                                <option value="donación" {{ old('reason') == 'donación' ? 'selected' : '' }}>Donación</option>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('exit_date') is-invalid @enderror" 
                                   ...
                                   value="{{ old('exit_date', date('Y-m-d')) }}" required>
                            @error('exit_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
